...
import type { JsonValue } from "../_data/logs-data"
...
/**
 * components/patterns/json-tree/json-tree-demo.tsx를 props 기반으로 일반화한
 * 재귀 JSON 트리 뷰어 — 접기/펼치기, 타입별 색, 대용량 배열 페이지네이션 동일.
 */
export function JsonTree({ label, data }: Readonly<{ label: string; data: JsonValue }>) {
  return (
    <div className="w-full overflow-x-auto rounded-md border border-border bg-muted/20 p-3">
      <JsonNode label={label} value={data} depth={0} defaultExpanded />
    </div>
  )
}
